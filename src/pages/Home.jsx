import { RefreshCw, Plus, Activity, CheckCircle, Clock, BookOpen, Calendar, MapPin, ArrowRight, Trash2, Eye, EyeOff, Bell, BellOff, BellRing } from 'lucide-react';
...
export default function Home() {
  ...
  // Actualizar estado si cambia desde ajustes del OS
  useEffect(() => {
  ...
  const handleActivateNotifications = useCallback(async () => {
    if (!('Notification' in window) || !('serviceWorker' in navigator) || !('PushManager' in window)) {
      setActivationMsg('⚠️ Tu navegador no soporta notificaciones push.');
      return;
    }
    if (!user || user.id.startsWith('user-local-')) {
      setActivationMsg('⚠️ Inicia sesión con tu cuenta para activar alertas.');
      return;
    }
    ...
    try {
      // 1. Solicitar permiso (requiere gesto del usuario — por eso usamos un botón)
      const permission = await Notification.requestPermission();
      setNotifPermission(permission);

      if (permission !== 'granted') {
        setActivationMsg('❌ Permiso denegado. Ve a Ajustes del navegador → Notificaciones → Permitir para este sitio.');
        setIsActivating(false);
        return;
      }

      // 2. Suscribir al PushManager con las claves VAPID
      const vapidKey = import.meta.env.VITE_VAPID_PUBLIC_KEY;
      if (!vapidKey) {
        setActivationMsg('⚠️ Clave VAPID no configurada. Contacta al administrador.');
        setIsActivating(false);
        return;
      }
      ...
      if (error) {
        console.error('[AURA] Error guardando suscripción push:', error);
        setActivationMsg('⚠️ Suscripción activada localmente pero falló al guardarse. Intenta de nuevo.');
      } else {
        setActivationMsg('✅ ¡Alertas académicas activadas! Recibirás notificaciones aunque la app esté cerrada.');
        console.log('[AURA] Push subscription guardada exitosamente:', subJson.endpoint);
      }
    } catch (err) {
      console.error('[AURA] Error activando notificaciones:', err);
  ...
  return (
    <main className="main-content">
      ...
      {/* ── Banner de Activación de Alertas Push ── */}
      {notifPermission !== 'granted' && notifPermission !== 'unsupported' && (
        <div style={{
          ...
          boxShadow: '0 4px 24px rgba(139, 92, 246, 0.12), inset 0 1px 0 rgba(255,255,255,0.08)',
          flexWrap: 'wrap'
        }}>
          <div style={{
          ...
          <div style={{ flex: 1, minWidth: '200px' }}>
            <p style={{ margin: 0, fontWeight: 700, fontSize: '1rem', color: 'var(--text-main)' }}>
              Activar alertas académicas
            </p>
            <p style={{ margin: '3px 0 0', fontSize: '0.82rem', color: 'var(--text-muted)', lineHeight: 1.4 }}>
              Recibe avisos de clases, bloques de estudio y chats aunque la app esté cerrada o en segundo plano.
            </p>
            {activationMsg && (
              <p style={{ margin: '6px 0 0', fontSize: '0.82rem',
                color: activationMsg.startsWith('✅') ? '#10b981' : activationMsg.startsWith('❌') ? '#ef4444' : '#f59e0b',
                fontWeight: 600 }}
              >{activationMsg}</p>
            )}
          </div>
          <button
            id="btn-activar-alertas"
            onClick={handleActivateNotifications}
            disabled={isActivating}
            style={{
              display: 'flex', alignItems: 'center', gap: '8px',
              padding: '12px 22px', borderRadius: '12px', border: 'none', cursor: isActivating ? 'wait' : 'pointer',
              fontWeight: 700, fontSize: '0.9rem', flexShrink: 0, transition: 'all 0.2s ease',
              background: isActivating
                ? 'rgba(139,92,246,0.4)'
                : 'linear-gradient(135deg, #8b5cf6, #6366f1)',
              color: 'white',
              boxShadow: isActivating ? 'none' : '0 4px 14px rgba(139, 92, 246, 0.45)'
            }}
            onMouseEnter={e => { if (!isActivating) e.currentTarget.style.transform = 'translateY(-2px)'; }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; }}
          >
            {isActivating ? (
              <><RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> Activando...</>
            ) : (
      ...
      {/* Confirmación cuando ya están activas */}
      {notifPermission === 'granted' && activationMsg.startsWith('✅') && (
        <div style={{
      ...
      {/* Banner si fue denegado en ajustes del browser */}
      {notifPermission === 'denied' && (
        <div style={{
          ...
          <BellOff size={20} color="#ef4444" style={{ flexShrink: 0 }} />
          <div>
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#ef4444', fontWeight: 700 }}>Notificaciones bloqueadas en el navegador</p>
            <p style={{ margin: '2px 0 0', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
              Ve a Ajustes del navegador → Privacidad y seguridad → Notificaciones → y permite este sitio.
            </p>
          </div>
        </div>
      )}
